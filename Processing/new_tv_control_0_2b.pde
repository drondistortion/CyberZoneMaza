/*
 * Программа для Processing (f-ing google it)
 ...
 * стиль кода - говно, т.к. его сочинял механик,
 * а не программист. Все переменные public, потому что
 * я трахал писать методы для каждой операции над ними.
 * Так же не помню последняя это версия или нет.
 * Но не сложно разобраться по ходу пьесы,
 * достаточно найти человека, желающего поаутировать
 * над кодом.
 */
